...
type TransactionServiceInterface interface {
	...
	//   - txHash: 交易哈希（32字节的十六进制字符串，以0x开头）
	// 返回:
	...
	//   - *domain.Transaction: 包含交易完整信息的领域模型指针
	//   - error: 如果查询过程中发生错误，将返回相应的错误信息
	GetTransactionByIndex(ctx context.Context, blockHashOrNumber string, index uint64) (*domain.Transaction, error)
	...
	// 返回:
	//   - string: 交易哈希（32字节的十六进制字符串，以0x开头）
	...
	// 参数:
	//   - ctx: 上下文对象，用于控制请求的生命周期
	//   - address: 以太坊账户地址（20字节的十六进制字符串，以0x开头）
	//   - blockHashOrNumber: 区块标识符，可以是区块号（数字字符串）或区块哈希（0x开头的十六进制字符串）
	//     支持的特殊值："latest"（最新区块）、"earliest"（创世区块）、"pending"（待打包区块）
	// 返回:
	//   - uint64: 交易数量（非负整数）
